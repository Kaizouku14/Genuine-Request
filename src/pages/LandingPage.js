...
export const LandingPage = () => {
  ...
  return (
    <div className={`relative h-screen flex justify-center items-center overflow-hidden bg-cover transition-background duration-500`} style={{ backgroundImage }}>
      <audio ref={audioRef} src={isSad ? sadaudio : ''} preload="auto" /> 
      ...
      <div className='relative z-10 flex flex-col items-center'>
        <img 
          ...
          alt='teddy' 
        />
        <h1 className='font-poppins text-4xl font-bold text-pink-700 '>Hello my dear</h1>
        <p className='text-pink-800 text-lg font-medium mt-2'>Would you like to go out with me for a date?</p>
        <div className='flex gap-x-4 mt-4'>
          <button 
            className="relative w-28 bg-pink-500 text-white font-semibold rounded-full py-2 shadow-lg transition-transform duration-300 ease-in-out hover:scale-105 hover:bg-pink-600 focus:ring-4 focus:ring-pink-300 focus:outline-none group"
            onClick={() => {
              setIsSad(false);
